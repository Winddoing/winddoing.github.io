...
* `cpuset`: 为cgroup中的任务分配独立CPU（SMP多核）和内存节点
* `cpu`： 提供调度程序对CPU的cgroup任务访问
* `cpuacct`： 自动生成cgroup中任务所使用的CPU报告
* `devices`: 允许或拒绝cgroup中的任务访问设备
* `freezer`：挂起或者恢复cgroup中的任务
* `blkio`： 块设备输入输出的限制
* `perf_event`：  增加了对每group的监测跟踪的能力，即可以监测属于某个特定的group的所有线程以及运行在特定CPU上的线程，此功能对于监测整个group非常有用，[https://lwn.net/Articles/421574/](https://lwn.net/Articles/421574/)
