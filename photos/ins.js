        ...
        var render = function render(res) {
            ...
            for (var j = 0, len2 = res.list.length; j < len2; j++) {
                ...
                for (var i = 0, len = data.link.length; i < len; i++) {
                    ...
                    var minSrc = 'https://winddoing.coding.net/p/photos-data/d/photos/git/raw/coding-pages/min_images/' + data.link[i];
                    var src = 'https://winddoing.coding.net/p/photos-data/d/photos/git/raw/coding-pages/images/' + data.link[i];
                    var type = data.type[i];
                    var target = src + (type === 'video' ? '.mp4' : '.jpg');
                    src += '';
                    liTmpl += '<figure class="thumb" itemprop="associatedMedia" itemscope itemtype="http://schema.org/ImageObject">\
                        <a href="' + src + '" itemprop="contentUrl" data-size="1920x1080" data-type="' + type + '" data-target="' + src + '">\
                          <img class="reward-img" data-type="' + type + '" data-src="' + minSrc + '" data-src="assets/empty.png" itemprop="thumbnail" onload="lzld(this)">\
                        </a>\
                        <figcaption style="display:none" itemprop="caption description">' + data.text[i] + '</figcaption>\
                    </figure>';
                }
                ulTmpl = ulTmpl + '<section class="archives album"><h2 class="year">' + data.year + '年<em>' + data.month + '月</em></h1>\
